...
from .receive import OpenTTDProtocolCoordinatorReceive
from .send import OpenTTDProtocolCoordinatorSend

log = logging.getLogger(__name__)


class OpenTTDProtocolTCPCoordinator(
    asyncio.Protocol, OpenTTDProtocolCoordinatorReceive, OpenTTDProtocolCoordinatorSend
):
    ...
    async def _process_queue(self):
        while True:
            ...
            try:
                ...
                log.exception(f"Internal error: receive_packet for {type.name} triggered an exception")
                self.transport.close()
                return
...
@click_helper.extend
@click.option(
    "--coordinator-proxy-protocol",
    help="Enable Proxy Protocol (v1), and expect all incoming package to have this header.",
    is_flag=True,
)
def click_coordinator_proxy_protocol(coordinator_proxy_protocol):
    OpenTTDProtocolTCPCoordinator.proxy_protocol = coordinator_proxy_protocol
